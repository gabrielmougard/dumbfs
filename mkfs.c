...
static struct superblock *write_superblock(int fd, struct stat *fstats) {
	...
	uint32_t nr_blocks = fstats->st_size / DUMBFS_BLOCK_SIZE;
	uint32_t nr_inodes = nr_block;
	uint32_t mod = nr_inodes % DUMBFS_INODES_PER_BLOCK;
	if (mod) nr_inodes += DUMBFS_INODES_PER_BLOCK - mod;
	uint32_t nr_istore_blocks =
	    idiv_ceil(nr_inodes, DUMBFS_INODES_PER_BLOCK);
	uint32_t nr_ifree_blocks = idiv_ceil(nr_inodes, DUMBFS_BLOCK_SIZE * 8);
	uint32_t nr_bfree_blocks = idiv_ceil(nr_blocks, DUMBFS_BLOCK_SIZE * 8);
	uint32_t nr_data_blocks = nr_blocks - 1 - nr_istore_blocks -
				  nr_ifree_blocks - nr_bfree_blocks;

	memset(sb, 0, sizeof(struct superblock));
	sb->info = (struct dumbfs_sb_info){
	    .magic = htole32(DUMBFS_MAGIC),
...
static int write_inode_store(int fd, struct superblock *sb) {
	...
	char *block = malloc(DUMBFS_BLOCK_SIZE);
	if (!block) return -1;

	memset(block, 0, DUMBFS_BLOCK_SIZE);

	/* Root inode (inode 0) */
	struct dumbfs_inode *inode = (struct dumbfs_inode *)block;
	...
	inode->i_mode =
	    htole32(S_IFDIR | S_IRUSR | S_IRGRP | S_IROTH | S_IWUSR | S_IWGRP |
		    S_IXUSR | S_IXGRP | S_IXOTH);
	inode->i_uid = 0;
	inode->i_gid = 0;
	inode->i_size = htole32(DUMBFS_BLOCK_SIZE);
	...
	int ret = write(fd, block, DUMBFS_BLOCK_SIZE);
	if (ret != DUMBFS_BLOCK_SIZE) {
		ret = -1;
		goto end;
	}
	...
	for (i = 1; i < sb->info.nr_istore_blocks; i++) {
		ret = write(fd, block, DUMBFS_BLOCK_SIZE);
		if (ret != DUMBFS_BLOCK_SIZE) {
			ret = -1;
			goto end;
		}
...
static int write_ifree_blocks(int fd, struct superblock *sb) {
	...
	for (i = 1; i < le32toh(sb->info.nr_ifree_blocks); i++) {
		ret = write(fd, ifree, DUMBFS_BLOCK_SIZE);
		if (ret != DUMBFS_BLOCK_SIZE) {
			ret = -1;
			goto end;
		}
...
static int write_bfree_blocks(int fd, struct superblock *sb) {
	 ...
	 */
	memset(bfree, 0xff, DUMBFS_BLOCK_SIZE);
	uint32_t i = 0;
	while (nr_used) {
	...
	int ret = write(fd, bfree, DUMBFS_BLOCK_SIZE);
	if (ret != DUMBFS_BLOCK_SIZE) {
		ret = -1;
		goto end;
	}
	...
	for (i = 1; i < le32toh(sb->info.nr_bfree_blocks); i++) {
		ret = write(fd, bfree, DUMBFS_BLOCK_SIZE);
		if (ret != DUMBFS_BLOCK_SIZE) {
			ret = -1;
			goto end;
		}
...
int main(int argc, char **argv) {
	...
	long int min_size = 100 * DUMBFS_BLOCK_SIZE;
