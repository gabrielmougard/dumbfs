#ifndef DUMBFS_BITMAP_H
#define DUMBFS_BITMAP_H
 ...
 */
static inline uint32_t get_free_inode(struct dumbfs_sb_info *sbi) {
	uint32_t ret =
	    get_first_free_bits(sbi->ifree_bitmap, sbi->nr_inodes, 1);
	if (ret) sbi->nr_free_inodes--;
	return ret;
}
 ...
 */
static inline uint32_t get_free_blocks(struct dumbfs_sb_info *sbi,
				       uint32_t len) {
	uint32_t ret =
	    get_first_free_bits(sbi->bfree_bitmap, sbi->nr_blocks, len);
	if (ret) sbi->nr_free_blocks -= len;
	return ret;
}
...
static inline void put_inode(struct dumbfs_sb_info *sbi, uint32_t ino) {
	if (put_free_bits(sbi->ifree_bitmap, sbi->nr_inodes, ino, 1)) return;

	sbi->nr_free_inodes++;
}

/* Mark len block(s) as unused */
static inline void put_blocks(struct dumbfs_sb_info *sbi, uint32_t bno,
			      uint32_t len) {
	if (put_free_bits(sbi->bfree_bitmap, sbi->nr_blocks, bno, len)) return;

	sbi->nr_free_blocks += len;
}

#endif /* DUMBFS_BITMAP_H */ 1
