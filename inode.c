...
static const struct inode_operations dumbfs_inode_ops;
...
struct inode *dumbfs_iget(struct super_block *sb, unsigned long ino) {
	...
	struct dumbfs_inode *cinode = NULL;
	struct dumbfs_inode_info *ci = NULL;
	...
	if (ino >= sbi->nr_inodes) return ERR_PTR(-EINVAL);
	...
	cinode = (struct dumbfs_inode *)bh->b_data;
	cinode += inode_shift;

	inode->i_ino = ino;
	inode->i_sb = sb;
	inode->i_op = &dumbfs_inode_ops;
	...
	if (S_ISDIR(inode->i_mode)) {
		...
		inode->i_fop = &dumbfs_dir_ops;
	} else if (S_ISREG(inode->i_mode)) {
		ci->ei_block = le32_to_cpu(cinode->ei_block);
		inode->i_fop = &dumbfs_file_ops;
		inode->i_mapping->a_ops = &dumbfs_aops;
...
static struct dentry *dumbfs_lookup(struct inode *dir, struct dentry *dentry,
	...
	struct dumbfs_inode_info *ci_dir = DUMBFS_INODE(dir);
	struct inode *inode = NULL;
	struct buffer_head *bh = NULL;
	...
	if (dentry->d_name.len > DUMBFS_FILENAME_LEN)
	...
	for (i = 0; i < DUMBFS_MAX_SUBFILES; i++) {
		f = &dblock->files[i];
		if (!f->inode) break;
		if (!strncmp(f->filename, dentry->d_name.name,
			     DUMBFS_FILENAME_LEN)) {
			inode = dumbfs_iget(sb, f->inode);
			break;
		}
	}
	brelse(bh);
...
static struct inode *dumbfs_new_inode(struct inode *dir, mode_t mode) {
	struct inode *inode;
	...
	if (!ino) return ERR_PTR(-ENOSPC);

	inode = dumbfs_iget(sb, ino);
	if (IS_ERR(inode)) {
		ret = PTR_ERR(inode);
		goto put_ino;
	}
	...
	if (S_ISDIR(mode)) {
		ci->dir_block = bno;
		inode->i_size = DUMBFS_BLOCK_SIZE;
		inode->i_fop = &dumbfs_dir_ops;
 ...
 */
static int dumbfs_create(struct inode *dir, struct dentry *dentry, umode_t mode,
			 bool excl) {
	struct super_block *sb;
	struct inode *inode;
	struct dumbfs_inode_info *ci_dir;
	struct dumbfs_dir_block *dblock;
	char *fblock;
	struct buffer_head *bh, *bh2;
	int ret = 0, i;

	/* Check filename length */
	if (strlen(dentry->d_name.name) > DUMBFS_FILENAME_LEN)
		return -ENAMETOOLONG;

	/* Read parent directory index */
	ci_dir = DUMBFS_INODE(dir);
	sb = dir->i_sb;
	bh = sb_bread(sb, ci_dir->dir_block);
	if (!bh) return -EIO;
	...
	if (dblock->files[DUMBFS_MAX_SUBFILES - 1].inode != 0) {
		ret = -EMLINK;
		goto end;
	}

	/* Get a new free inode */
	inode = dumbfs_new_inode(dir, mode);
	if (IS_ERR(inode)) {
		ret = PTR_ERR(inode);
		goto end;
	}
	...
	/*
	 * Scrub ei_block/dir_block for new file/directory to avoid previous
	 * data messing with new file/directory.
	 */
	bh2 = sb_bread(sb, DUMBFS_INODE(inode)->ei_block);
	if (!bh2) {
		ret = -EIO;
		goto iput;
	}
	fblock = (char *)bh2->b_data;
	memset(fblock, 0, DUMBFS_BLOCK_SIZE);
	mark_buffer_dirty(bh2);
	brelse(bh2);

	/* Find first free slot in parent index and register new inode */
	for (i = 0; i < DUMBFS_MAX_SUBFILES; i++)
		if (dblock->files[i].inode == 0) break;
...
iput:
	put_blocks(DUMBFS_SB(sb), DUMBFS_INODE(inode)->ei_block, 1);
	put_inode(DUMBFS_SB(sb), inode->i_ino);
	iput(inode);
end:
	brelse(bh);
	return ret;
...
static int dumbfs_unlink(struct inode *dir, struct dentry *dentry) {
	...
	struct dumbfs_dir_block *dir_block = NULL;
	struct dumbfs_file_ei_block *file_block = NULL;
	...
	if (f_id != DUMBFS_MAX_SUBFILES - 1)
		memmove(dir_block->files + f_id, dir_block->files + f_id + 1,
			(nr_subs - f_id - 1) * sizeof(struct dumbfs_file));
	memset(&dir_block->files[nr_subs - 1], 0, sizeof(struct dumbfs_file));
	mark_buffer_dirty(bh);
	brelse(bh);

	if (S_ISLNK(inode->i_mode)) goto clean_inode;
	 ...
	 */
	bno = DUMBFS_INODE(inode)->ei_block;
	bh = sb_bread(sb, bno);
	if (!bh) goto clean_inode;
	file_block = (struct dumbfs_file_ei_block *)bh->b_data;
	if (S_ISDIR(inode->i_mode)) goto scrub;
	for (i = 0; i < DUMBFS_MAX_EXTENTS; i++) {
		char *block;

		if (!file_block->extents[i].ee_start) break;
		...
		for (j = 0; j < file_block->extents[i].ee_len; j++) {
			...
			memset(block, 0, DUMBFS_BLOCK_SIZE);
			mark_buffer_dirty(bh2);
			brelse(bh2);
		}
	}
...
scrub:
	/* Scrub index block */
	memset(file_block, 0, DUMBFS_BLOCK_SIZE);
	mark_buffer_dirty(bh);
	brelse(bh);
...
static int dumbfs_rename(struct inode *old_dir, struct dentry *old_dentry,
			 struct inode *new_dir, struct dentry *new_dentry,
			 unsigned int flags) {
	struct super_block *sb = old_dir->i_sb;
	struct dumbfs_inode_info *ci_old = DUMBFS_INODE(old_dir);
	struct dumbfs_inode_info *ci_new = DUMBFS_INODE(new_dir);
	...
	if (flags & (RENAME_EXCHANGE | RENAME_WHITEOUT)) return -EINVAL;

	/* Check if filename is not too long */
	if (strlen(new_dentry->d_name.name) > DUMBFS_FILENAME_LEN)
		return -ENAMETOOLONG;

	/* Fail if new_dentry exists or if new_dir is full */
	bh_new = sb_bread(sb, ci_new->dir_block);
	if (!bh_new) return -EIO;
	dir_block = (struct dumbfs_dir_block *)bh_new->b_data;
	for (i = 0; i < DUMBFS_MAX_SUBFILES; i++) {
		...
		if (new_pos < 0 && dir_block->files[i].inode == 0) new_pos = i;
	}
	/* if old_dir == new_dir, just rename entry */
	if (old_dir == new_dir) {
		strncpy(dir_block->files[f_pos].filename,
			new_dentry->d_name.name, DUMBFS_FILENAME_LEN);
	...
	if (S_ISDIR(src->i_mode)) drop_nlink(old_dir);
...
static int dumbfs_mkdir(struct inode *dir, struct dentry *dentry,
			umode_t mode) {
	return dumbfs_create(dir, dentry, mode | S_IFDIR, 0);
}

static int dumbfs_rmdir(struct inode *dir, struct dentry *dentry) {
	struct super_block *sb = dir->i_sb;
	struct inode *inode = d_inode(dentry);
	...
	bh = sb_bread(sb, DUMBFS_INODE(inode)->dir_block);
	if (!bh) return -EIO;
	dblock = (struct dumbfs_dir_block *)bh->b_data;
	...
	return dumbfs_unlink(dir, dentry);
}

static int dumbfs_link(struct dentry *old_dentry, struct inode *dir,
		       struct dentry *dentry) {
	struct inode *inode = d_inode(old_dentry);
	struct super_block *sb = inode->i_sb;
	struct dumbfs_inode_info *ci_dir = DUMBFS_INODE(dir);
	struct dumbfs_dir_block *dir_block;
	struct buffer_head *bh;
	...
	if (dir_block->files[DUMBFS_MAX_SUBFILES - 1].inode != 0) {
		ret = -EMLINK;
...
static int dumbfs_symlink(struct inode *dir, struct dentry *dentry,
			  const char *symname) {
	struct super_block *sb = dir->i_sb;
	unsigned int l = strlen(symname) + 1;
	struct inode *inode = dumbfs_new_inode(dir, S_IFLNK | S_IRWXUGO);
	...
	if (l > sizeof(ci->i_data)) return -ENAMETOOLONG;
...
static const char *dumbfs_get_link(struct dentry *dentry, struct inode *inode,
